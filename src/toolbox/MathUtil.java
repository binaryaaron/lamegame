...
/**
 * 
 * Toolbox for Matrix math and manipulations. Used n camera calculations
 * 
 * @author Paige
 *
 */
public class MathUtil {
	...
	/**
	 * translation for global world objects. Takes Entity paramaters without
	 * taking entity. used for skyboxes. set up for other kinds of entities.
	 * 
	 * @param translation
	 * @param rx
	 * @param ry
	...
	public static Matrix4f createTransformationMatrix(Vector3f translation,
		...
		Matrix4f.rotate((float) Math.toRadians(0f), new Vector3f(1, 0, 0),
				matrix, matrix);
		Matrix4f.rotate((float) Math.toRadians(0f), new Vector3f(0, 1, 0),
				matrix, matrix);
		Matrix4f.rotate((float) Math.toRadians(rz), new Vector3f(0, 0, 1),
				matrix, matrix);
		// Matrix4f.scale(new Vector3f(scale, scale, scale), matrix, matrix);

		return createTransformationMatrix(new Entity("", null, translation, rx,
				ry, rz, scale));
		// return matrix;
	}

	/**
	 * creates transformation matrix for an input entity.
	 * @param entity
	 * @return
	 */
	public static Matrix4f createTransformationMatrix(Entity entity) {
		...
		Matrix4 viewMatrix = entity.orientation.toMatrix().translate(
				new Vector3(entity.position.x, entity.position.y,
						entity.position.z));
	...
	/**
	 * creates a view matrix for an input camera
	 * @param camera
	 * @return
	 */
	public static Matrix4f createViewMatrix(Camera camera) {
		...
		Matrix4 viewMatrix = camera.orientation.toMatrix().translate(
				new Vector3(camera.position.x, camera.position.y,
						camera.position.z));
	...
	/**
	 * parse ra4king Matrix4 into lwjgl matrix4f
	 * @param viewMatrix
	 * @return
	 */
